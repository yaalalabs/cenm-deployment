...
while true
do
    if [ ! -f certificates/nodekeystore.jks ]
    then
        ...
        echo
        echo "Notary Service: running initial registration ..."
        echo
        java -Dcapsule.jvm.args='-Xmx{{ .Values.cordaJarMx }}G' -jar {{ .Values.jarPath }}/ha-utilities.jar \
        notary-registration \
        ...
        echo
        echo "Notary Service: already registered to Identity Manager - skipping initial registration."
        echo
        EXIT_CODE="0"
        break
    fi
...
then
    echo
    echo "Notary service initial registration failed - exit code: ${EXIT_CODE} (error)"
    echo
